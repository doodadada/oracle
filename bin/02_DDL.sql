...
-- 도서대여점의 도서목록 테이블의 생성
-- 필드 : booknu, subject, makeyear, inprice, outprice
-- 자로형 : booknum(문자5자리),subject(문자30), makeyear(숫자4),
...
create table booklist(
	booknum varchar2(5),  --varchar2는 mysql에는 없음
...
drop table booklist purge; --purge는 휴지통을 거치지 않고 완전삭제를 의미

-- not null  : null을 허용하지 않는 제약. 해당 필드가 빈칸인 채로는 레코드가 insert 되지 않습니다.
...
-- 		CREATE TABLE
--		Create Table
-- 9. 보통은 데이터형색과 용량(크기)를 지정하는데, DATE 데이터형식은 유형을 별도로 크기를 지정하지 않음
...
-- 13. 컬럼(필드)과 칼럼(필드)의 구분은 콤마로 하되, 마지막 컬럼이나 constraint 를 작성후엔 콤마를 찍지 않음
...
-- 상대 테이블의 특정 필드값을 참조하면서 없는 값을 사용할 수 없도록 하는 키,
-- 외래키로 유지되는 무결성을 "참조무결성" 이라고 부릅니다.
-- 예를 들면 booklist에 존재하지 않은 도서의 번호가 rentlist의 빌려간 도서의 번호로 등록되지 못하게 하는 것을 말합니다.
...
create table rentlist(
	...
	--constraint rent_pk primary key(rent_date, rent_seq) 값 두개를 조합해서 기본키로도 사용 가능함
);
