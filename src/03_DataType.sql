...
-- 		varchar2(xx)
--		지정한 숫자를 저장할 수 있는 최대 글자수를 지정하고 할당되는 용량은 실제 글자수만큼 할당되어 저장합니다.
-- 		varchar2(10) 에 "abcd"를 저장하면 4글자만큼의 용량만 소비됩니다.

-- 4. 고정형 문자(최대 2000 Byte)
--		char(xx)
--		char(10) 에 "abcd"를 저장하면 "abcd      "가 저장(빈칸이 실제 생긴다는 것은 아님)
...
--		timestamp : java.sql.Timestamp 자료형과 같은 형식		년월일시분초+밀리초까지 표현이 가능합니다.
-- 		- 가장 일반적이고 많이 사용하는 날짜 데이터 타입은 DATE
--		sysdate : date 형식의 오늘 날짜 현재시간을 발행하는 키워드
--		systimestamp : timestamp 형식의 오늘날짜 현재시간(밀리초 포함)을 발행하는 키워드 
